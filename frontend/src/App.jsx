      ...
      {user ? (
        <>
          <div className="hidden md:flex gap-6 items-center">
            {user.role === 'RECRUITER' ? (
              <Link to="/recruiter" className="text-muted-foreground hover:text-primary transition-colors text-sm font-medium">Dashboard</Link>
            ) : (
              <Link to="/candidate" className="text-muted-foreground hover:text-primary transition-colors text-sm font-medium">Dashboard</Link>
            )}
          </div>
          
          <div className="flex gap-4 items-center">
            <button onClick={logout} className="text-muted-foreground hover:text-primary transition-colors text-sm font-medium">Logout</button>
          </div>
        </>
      ) : (
        <>
